"""
Exercise1 — Foundations of Data Science
...
def load_and_clean(csv_path: str) -> pd.DataFrame:
    ...
    df['total_guests'] = df[['adults', 'children', 'babies']].sum(axis=1, min_count=1)
...
def categorical_cancel_stats(df: pd.DataFrame) -> Dict[str, object]:
    ...
    hotel_rates = df.groupby('hotel')['is_canceled'].mean().to_dict()
    
    # TODO 2 — deposit_rates
    
    deposit_rates = df.groupby('deposit_type')['is_canceled'].mean().to_dict()
    
    # TODO 3 — top_segment_500
    
    segment_counts = df['market_segment'].value_counts()
    
    valid_segments = segment_counts[segment_counts >= 500].index
    
    segment_rates = df[df['market_segment'].isin(valid_segments)].groupby('market_segment')['is_canceled'].mean()
    
    top_segment = segment_rates.idxmax()
    
    top_rate = segment_rates.max()
    ...
    return {
        ...
        'top_segment_500': (top_segment, top_rate)
    }
